...
func NewSync(ctx context.Context) *SYNC {
	s := &SYNC{
		...
		mu:          sync.Mutex{},
		CurrentView: 0,
		max:         dependency.Configs.Network.MAX_Timeout,
		timeoutMul:  1,
...
func (s *SYNC) handleTimeout() {
	...
	// 重置超时倍数
	if s.timeoutMul < dependency.Configs.Network.MAX_Timeout/dependency.Configs.Network.BASE_Timeout {
		s.timeoutMul *= 2
	} else {
		// 达到最大倍数时，保持不变或设置为最大超时倍数的值
		s.timeoutMul = dependency.Configs.Network.MAX_Timeout / dependency.Configs.Network.BASE_Timeout
	}
